...
export default function SeriesDetail() {
  ...
  if (loading) return <div className="flex min-h-[60vh] items-center justify-center text-gray-400">Loading series...</div>;
  if (error) return <div className="rounded-xl bg-red-950 p-8 text-center text-red-300">{error}</div>;
  if (!series) return <div className="py-20 text-center text-gray-400">Series not found.</div>;

  return (
    <div className="mx-auto max-w-7xl">
      <DetailHero movie={series} mediaType="tv" onTrailerClick={() => setTrailerOpen(true)} hasTrailer={Boolean(trailerKey)} />
      <section className="mt-12"><h2 className="mb-5 text-2xl font-bold">Cast</h2><CastRow cast={cast} /></section>
      <MovieRow title="Similar Series" movies={similar} mediaType="tv" />
      {trailerOpen && <TrailerModal videoKey={trailerKey} onClose={() => setTrailerOpen(false)} />}
    </div>
  );
}
